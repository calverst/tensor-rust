...
// Q4_K_M: B matrix stored as block_q4_K blocks (144 bytes per 256 elements).
// Forces SB=1 because dequantization needs a local staging buffer.
#ifdef Q4K_B
#undef SB
#define SB 1
...
// Dequantize one element from a block_q4_K.
// block layout: [d:f16][dmin:f16][scales:u8×12][qs:u8×128]
float dequant_q4k(__global const uchar* block, int elem_idx)
{
    float d    = vload_half(0, (__global const half*)block);
    float dmin = vload_half(1, (__global const half*)block);

    __global const uchar* scales = block + 4;
    __global const uchar* qs     = block + 16;

    int sub = elem_idx >> 5;   // sub-block index 0..7
    uchar sc, mn;
    if (sub < 4) {
        sc = scales[sub]     & 63;
        mn = scales[sub + 4] & 63;
    ...
    uchar q_byte = qs[elem_idx >> 1];
    uchar q4 = (elem_idx & 1) ? (q_byte >> 4) : (q_byte & 0xF);

    return d * (float)sc * (float)q4 - dmin * (float)mn;
}

// Copy and dequantize a KWG×NWG tile of B into local half memory.
// bgm_blocks: per-batch base pointer; one block_q4_K per column (K==QK_K==256).
void GlobalToLocalB_q4k(int tid, int n_start, int k_start,
                         __local short* blm,
                         __global const uchar* bgm_blocks)
{
    const int copy_size   = KWG * NWG;
    const int num_threads = MDIMC * NDIMC * 32 / 256;
    ...
    for (int i = tid; i < copy_size; i += num_threads) {
        ...
        __global const uchar* block_ptr = bgm_blocks + n_global * BLOCK_Q4_K_SIZE;
        float val = dequant_q4k(block_ptr, k_global);
        ((__local half*)blm)[i] = (half)val;
    }
}
...
void HgemmBody(const int kSizeM, const int kSizeN, const int kSizeK,
                  ...
                  #ifdef Q4K_B
                  const __global uchar* restrict bgm,
                  #else
...
#ifdef Q4K_B
        GlobalToLocalB_q4k(
            get_local_id(0) + get_local_id(1) * 32 * MDIMC,
            get_group_id(1) * NWG,
            kwg,
            blm,
            bgm
                  ...
                  __global half* restrict cgm)
{
    ...
    #ifdef Q4K_B
    // One block_q4_K per column per batch (K == QK_K == 256)
    const int b_offset = kSizeN * BLOCK_Q4_K_SIZE * batch;
    #else
    ...
    #ifdef Q4K_B
    const __global uchar* restrict bgm_ = &bgm[b_offset];
    #else
